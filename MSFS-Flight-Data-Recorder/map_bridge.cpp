...
void MapBridge::saveTakeoffAnalysisReport(int rowId, const QString& report) {
	if (rowId <= 0) {
		Logger::logf(Logger::Trace, "DB", "saveTakeoffAnalysisReport: ignoring invalid takeoff id %d", rowId);
		return;
	}
	sqlite3* sql = connect_db_readwrite();
	if (!sql) {
		Logger::logf(Logger::Warning, "DB", "saveTakeoffAnalysisReport(takeoff %d): failed to open read-write connection; analysis was not saved", rowId);
		return;
	}
	const char* q = "UPDATE trip_takeoffs SET analysis_report = ? WHERE id = ?";
	...
	if (sqlite3_prepare_v2(sql, q, -1, &stmt, nullptr) == SQLITE_OK) {
		...
		if (sqlite3_step(stmt) != SQLITE_DONE)
			Logger::logf(Logger::Warning, "DB", "saveTakeoffAnalysisReport(takeoff %d): update failed: %s", rowId, sqlite3_errmsg(sql));
		else
			Logger::logf(Logger::Trace, "DB", "saveTakeoffAnalysisReport(takeoff %d): analysis report saved (%d chars)", rowId, utf8.size());
		sqlite3_finalize(stmt);
	} else {
		Logger::logf(Logger::Warning, "DB", "saveTakeoffAnalysisReport(takeoff %d): prepare failed: %s", rowId, sqlite3_errmsg(sql));
	}
	sqlite3_close(sql);
}
